...
const functions = require("../src/sql-server-functions");
...
describe("Tests de funciones SQL Server", () => {
  describe("Create", () => {
    it("Creación fallida - Sin Body", () => functions.create_city().then((result) => {
      expect(result.error).to.eql(DB_ERRORS.PARAMETERS_VALIDATION_ERROR);
    }));

    it("Creación fallida - Body vacío", () => functions.create_city({}).then((result) => {
      expect(result.error).to.eql(DB_ERRORS.PARAMETERS_VALIDATION_ERROR);
    }));

    it("Creación fallida - Body null", () => functions.create_city(null).then((result) => {
      expect(result.error).to.eql(DB_ERRORS.PARAMETERS_VALIDATION_ERROR);
    }));
    ...
    it("Creación exitosa", () => functions.create_city(newCity)
      .then((result) => {
        ...
        expect(result.payload[0].ZipCode).to.eql(newCity.ZipCode);
        expect(result.payload[0].CityName).to.eql(newCity.CityName);
  ...
  describe("Update", () => {
    ...
    it("Update fallido - Body vacío", () => functions.update_city(fakeZipCode, {}).then((result) => {
      expect(result.error).to.eql(DB_ERRORS.PARAMETERS_VALIDATION_ERROR);
      expect(result.payload).to.be.null;
    }));
    ...
    it("Update exitoso", () => functions.update_city(fakeZipCode, {
      ...
      expect(result.error).to.eql(DB_ERRORS.NO_ERROR);
      expect(result.payload).to.be.array;
      expect(result.payload[0].CityName).to.eql(fakeCityName2);
